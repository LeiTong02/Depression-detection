...
def random_forest(train, test, max_depth, min_size, sample_size, n_trees, n_features):
    ...
    trees = list()
    print('max_depth = %f, min_size=%f'%(max_depth,min_size))
    # n_trees 表示决策树的数量
...
def evaluate_algorithm(dataset, algorithm, n_folds, *args):
    ...
    for index,fold in enumerate(folds):
        train_set = [ff for index2,ff in enumerate(folds) if index2!=index]
...
if __name__ == '__main__':
    ...
    # print dataset
    print(gini_index(np.array([[1.5, 2.1, 3.8, 2.0], [2.0, 4.0, 4.0, 2.0], [3.0, 6.0, 4.0, 3.0]], dtype=np.float64),
                     np.array([[10.0, 12.0, 14.0, 4.0], [12.0, 13.0, 9.0, 4.0], [18.0, 34.0, 12.0, 4.0]],
                              dtype=np.float64)))
    ...
    # n_features = int((len(dataset[0])-1))
         # 调参（自己修改） #准确性与多样性之间的权衡

    iris = datasets.load_iris()
    X= iris.data
    y=iris.target

    dataset = np.insert(X,4,y,axis=1).tolist()
    n_features = int(math.sqrt(X.shape[1]))
